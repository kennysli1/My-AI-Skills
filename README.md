...
| 技能目录 | 功能简介 |
|----------|----------|
| **algorithmic-art** | 用 **p5.js** 做算法艺术：种子随机、粒子/流场、参数可调。先写「算法哲学」(.md)，再实现为可交互 HTML。适合：生成艺术、粒子系统、流场、代码艺术。 |
| **brand-guidelines** | 为任意产物应用 **Anthropic 官方品牌**：主色/强调色、Poppins+Lora 字体、标题与正文样式。适合：需要品牌一致性的文档、幻灯片、页面。 |
| **canvas-design** | 用设计哲学做**静态视觉设计**：先写视觉哲学 (.md)，再在画布上产出 .png/.pdf。适合：海报、艺术图、品牌视觉，强调工艺感与极简文字。 |
| **claude-api** | 使用 **Claude API / Anthropic SDK** 开发应用：单次调用、流式、Tool Use、Agent SDK、多语言 (Python/TS/Java/Go 等)。触发：代码里用 `anthropic` 或用户提到 Claude API。 |
| **doc-coauthoring** | **文档共创流程**：上下文收集 → 逐段精修与结构 → 用「读者测试」验证可读性。适合：技术文档、提案、PRD、决策文档、RFC。 |
| **docx** | **Word 文档**：用 docx-js 新建 .docx，或 unpack → 改 XML → pack 编辑已有文档。支持样式、表格、图片、目录、修订与批注。 |
| **internal-comms** | **内部沟通写作**：3P 更新、周报、FAQ、事故报告、项目更新等，按公司格式与范例 (examples/) 撰写。 |
| **mcp-builder** | 开发 **MCP (Model Context Protocol) 服务器**：工具设计、TypeScript/Python 实现、测试与评估。适合：把外部 API/服务接入 LLM。 |
| **pdf** | **PDF 处理**：读取/提取文字表格、合并/拆分、旋转、水印、新建 PDF、填表、加解密、OCR。用 pypdf、pdfplumber、reportlab 等。 |
| **pptx** | **PPT 幻灯片**：用 markitdown 读内容、thumbnail 看版式；编辑用 unpack→改→pack；从零做用 pptxgenjs。含设计建议与 QA 流程。 |
| **skill-creator** | **创建与优化 Skill**：从意图到 SKILL.md 草稿、写测试用例、跑评测、根据反馈迭代、可选描述优化与打包。适合：做新 skill 或改进现有 skill。 |
| **slack-gif-creator** | 做 **Slack 用动图**：尺寸/帧率/颜色约束、GIFBuilder、校验、缓动与帧辅助。用 PIL 画图，支持抖动/脉冲/旋转/粒子等动画思路。 |
| **theme-factory** | 为幻灯片/文档/落地页等**套用主题**：10 套预设主题（颜色+字体），或按需求生成新主题，再应用到已有产物。 |
